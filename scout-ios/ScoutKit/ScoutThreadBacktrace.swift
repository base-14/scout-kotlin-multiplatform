...
/// reading its thread state via `thread_get_state`, and walking the frame-pointer chain,
/// symbolicating each return address with `dladdr`. Adapted verbatim from scout-flutter.
enum ScoutThreadBacktrace {
    ...
    private static func symbolicate(_ address: UInt) -> String {
        ...
        var info = dl_info()
        if dladdr(UnsafeRawPointer(bitPattern: address), &info) != 0, let namePtr = info.dli_sname {
